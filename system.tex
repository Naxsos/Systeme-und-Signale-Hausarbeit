Jede Differentialgleichung bekommt Anfangswerte


Abbildungsbenennungen
...
\begin{align*}
    ...
    x(t) & = \int_{0}^{t} \begin{bmatrix}e^{-t} * e^{\tau}\end{bmatrix} \,d\tau \\
    x(t) & = e^{-t} * e^{t} - e^{-t} * e^{0} \\
    x(t) & = 1 - e^{-t}
\end{align*}
...
Aus unserem physikalischen Modell lässt sich folgende Differenzialgleichung ableiten:
\begin{align*}
...
Vergessen werden darf auch hier nicht der Anfangswert. Es gilt:
\begin{align*}
    y(0) &= 0 
\end{align*}
...
Die Gewichtsfunktion $g(t)$ ist die Antwort eines Sytems auf einen Dirac-Impuls $\delta(t)$.\\
...
Die Spungantwort bezeichnet die abgetastete Antwort des Systems auf den technischen Einheitssorung.
Übergangsfunktion $h(t)$  ist die Antwort des Systems auf den Einheitssprung $\sigma (t)$.
...
Beim Bode-Plot lässt sich das entsprechende Verhalten anhand des Verlaufs zu Beginn des Plots feststellen. Bei P-Verfahlten wäre der Verlauf eine waagerechte Gerade, beim I-Verhalten eine Gerade mit negativer Steitung, beim D-Verhalten eine Gerade mit positiver Steigung. Im Bode-Plot ist eine waagerechte Linie zu erkennen. Das impliziert ein P-Verhalten, was mit dem Verhalten unseres System korrespondiert.
...
\begin{figure}[H]
 ...
 \begin{figure}[H]
    ...
    \caption{Theresa}
 \end{figure}
Die beiden abgebildeten Simulink-Schlatungen sind äquivalent und haben somit bei gleicher Eingangsfunktionen die gleiche Ausgangfunktion.
Mit einem Klick auf den Oszilator wird diese Ausgangsfunktion, sowie die am Eingang anliegende Sinus-Funktion in Form eins Graphen ausgegeben (Siehen Abbildung 10 und 11). Dabei ist die blaue Linie die Eingangs-Sinus-Funktion mit einer entsprechenden Frequenz $\omega$ in$\frac{rad}{s}$ und die Rote Linie der Ausgang $y(t)$ unseres Systems. 
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{Bilder/SimulinkOmega2.png}
    \caption{Ausgangsfunktion bei $\omega = 2$}
 \end{figure}
 \begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{Bilder/SimulinkOmega6.png}
    \caption{Ausgangsfunktion bei $\omega = 6$}
 \end{figure}

\subsubsection{Vergleich Bode-Plot, Simulink und Handrechnung}
Berechnung des Stationäre Anteils des Ausgangssignals. 
\begin{align*} 
    y_{stat} = |\mathcal{G}(j\omega_0)| \cdot A \cdot sin(\omega_0 t+\phi_0 + arg \enspace \mathcal{G}(j\omega))
\end{align*}
$|\mathcal{G}(j\omega_0)|$ entspricht der Amplitudenverstärkung und $arg \enspace \mathcal{G}(j\omega)$ der Phasenverschiebung.
\subsubsection*{Handrechnung}
\begin{align*} 
    \mathcal{G}(s) &=  \frac{1}{s+1}\cdot e^{-2s} \\
    \mathcal{G}(j\omega) &=  \frac{1}{j\omega+1} \cdot e^{-2j\omega} \\
\end{align*} 
Berechnung der Amplitudenverstärkgung für $ \omega = 6$: \\
\begin{align*} 
...
Berechnung der Phasenverschiebung für $ \omega = 6$: 

Es gilt: $e^{-aj} = -a$
\begin{align*} 
    arg \enspace \mathcal{G}(j2) &=   0- arctan \Bigl( \frac{6}{1} \Bigl) -12  \\
    arg \enspace \mathcal{G}(j2) &=  -1.4-12 = -13.4 \\
             \Rightarrow  -13.4 \cdot \frac{180}{3,14} & = -768.726 \approx -769^{\circ}
\end{align*} 
\subsubsection*{Vergleich}
Im Bodeplot lässt sich im oberen Bild für $\omega = 6$ eine Verstärkung von $-15.7 dB$ ablesen. Daraus lässt sich der Verstärkungsfaktor errechnen:
\begin{align*}
    20 log(\mathcal{K}) &= - 15.7\\
        \mathcal{K} &= 10^{-\frac{15.7}{20}}\\
        \mathcal{K} & = 0.1640
\end{align*}
Der aus der Übertragungsfunktion errechnete Wert von $1.644$ entspricht mit einer minimalen Abweichung dem Wert, der sich aus dem Bode-Diagramm ablesen lässt. 
...
\begin{figure}[H]
    ...
    \caption[width=0.6\textwidth]{Messung der Aplitude der eingeschwungnen \\Ausgansfunktion liegt ungefäht bei 0.17}
 \end{figure}

Im Bodeplot lässt sich im unteren Bild für $\omega = 6$ ein Winkel von $-769^{\circ}$ ablesen. Dieser Wert stimmt mit der analytischen Rechnung überein.

Die Phasenverschiebung in Grad wird berechnet durch $\phi = (\frac{\Delta t}{T} +T_t ) \cdot 360$, wobei $T$ die Periodendauer ist, die mit Hilfe der Formel $T = \frac{2\pi}{\omega} = \frac{2\pi}{6} = \frac{\pi}{3}$ ausgerechnet werden kann.
...
\begin{figure}[H]
    ...
    \caption[width=0.6\textwidth]{Messung der Zeitdifferenz zwischen der \\ Sinusschwingung und der eingeschwungnen Ausgansfunktion}
 \end{figure}

Alle Werte werden in die Formel eingsetzt:
\begin{align*}
    \phi &= (\frac{\Delta t}{T} +T_t ) \cdot 360 \\
    \phi &= (\frac{0.142 \cdot 3}{\pi} +2 ) \cdot 360 \\
    \phi &= 768.81^{\circ} \approx 769^{\circ}
\end{align*}
...
Da es sich bei unserem System um ein System mit proportionalen Übertragungsverhalten handelt, entspricht die statische Verstärkung $K = G(0)$ bzw. $K = \lim_{t\to \infty} h(t)$. Diese lässt sich in Matlab auch mit dem Befehl \texttt{dcgain(sys)} berechnen. 
...
Bei Systemen mit einem Eingang und zwei Ausgängen verwendet man stattdessen ein 2-Balken-Diagramm, bei zwei Eingängen und einem Ausgang wird die eine Eingangsgröße diskret und die andere kontinuierlich dargestellt, wodurch sich eine Funktionenschar ergibt. 
...
Abbildung 9 zeigt den Pol-Nulstellen-Plot zu unserem System. Der passende Matlab-Befehl hierfür lautet \texttt{pzplot(sys)}. Matlab markiert mit \textbf{o} die Nullstellen und mit \textbf{x} die Polstellen des Systems (doppelte Polstellen mit doppel-\textbf{x}). \\
...
Aufgrund dieses negativen Realteils handelt es sich also um eine grenzstabile Polstelle. Würden alle Polstellen unseres Systems in der offenen linken Halbebene liegen, wäre das System stabil. Da die Polstelle genau auf der Imaginärachse liegt, nennt man das System grenzstabil.
...
\begin{figure}[H]
 ...
 In unserem System ist das ziemlich simple: Unsere System-Matrix $A$ enthält nur das Element $-1$ und passt somit zur Polstelle $s = -1$.
