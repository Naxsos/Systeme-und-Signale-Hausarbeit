...
Mit dem MATLAB-Befehl \texttt{ss(sys)} (ss steht für State Space) berechnet MATLAB automatisch die Werte für unser System. Somit sieht die Zustandsraumdarstellung für unser System folgendermaßen aus:
\begin{align*}
    \dot x & = \begin{bmatrix}
        -1
...
\[
    G(s) = C(s * I - A)^{-1} * B + D
\]
Dies lässt sich auch in MATLAB eingeben, indem man man die Matrizen $A, B, C, D$ belegt und den Befehl \texttt{sys = ss(A, B, C, D)} ausführt. Von hier aus liefert der Befehl \texttt{tf(sys)} die Übertragungsfunktion.
...
Darüber hinaus gibt es einen Zusammenhang mit der Eingangs- / Ausgangs-Differentialgleichung: Die Anfangswerte des Zustandsraumes $x$ müssen mit denen der Eingangs- / Ausgangs-Differentialgleichung $y$ korrespondieren. Da wir bei unserem System davon anfangs kein Zufluss vorliegt, ist $u(0^-) = 0$. Daher lassen sich die Anfangswerte der mit folgender Formel berechnen:
...
Weiterhin gehen wir bei unserem System davon aus, dass der Tank zu Beginn leer ist, somit kein Ausfluss vorliegt. Daraus ergibt sich $x(0^-) = 0$ und mit voriger Formel gilt:
\begin{align*}
...
Aufgrund unseres physikalischen Modells gilt:
\begin{align*}
    y(t) & = x(t - 2)
\end{align*}
...
In Abbildung \ref{fig:impuls} ist die Reaktion des Systems auf technischen Eingangssprung zu sehen. Diese Gewichtsfunktion oder auch Impulsantwort bezeichnet man oft mit $\delta(t)$.
\\
...
Man bezeichnet diese Übergangsfunktion oder auch Sprungantwort oft mit $1(t)$.\\
In MATLAB lässt sich das mit dem Befehl \texttt{step(sys)} simulieren, wodurch man die numerische Lösung erhält. Die Übergangsfunktionfunktion $h(t)$ lässt sich aber auch analytisch berechnen und man erhält die explizite Lösung. Hierbei hilft einem der Befehl \texttt{ilaplace()}, der die Laplace-Transformierte zurückliefert:
...
Das bedeutet, das für jede Frequzenz $\omega$ eine komplexe Zahl $j$ mit Real- und Imaginärteil oder mit Betrag und Phase darstellt. Da in der Systemtheorie $i$ für den Strom steht, wird hier $j$ als imaginäre Einheit verwendet.

\subsubsection{Ortskurve (Nyquist-Plot)}
