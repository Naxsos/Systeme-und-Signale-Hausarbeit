...
Vergessen werden darf auch hier nicht der Anfangswert $y(0) = 0$.

\subsection{Übertragungsfunktion}

Die Übertragungsfunktion wird für unser System in Kapitel 2.3 definiert und lautet: 
...
Die Spungantwort bezeichnet die abgetastete Antwort des Systems auf den technischen Einheitssprung.
Die Übergangsfunktion $h(t)$ ist die Antwort des Systems auf den Einheitssprung.
...
\begin{figure}[H]
 ...
 \begin{figure}[H]
    ...
    \caption{Darstellung des Systems anhand der Ein-/Ausgangs-Differentialgleichung (Anfangswert des Integrators: $y(0) = 0$)}
 \end{figure}
...
Im Folgenden werden die Phasenverschiebung und Amplitudenverstärkung unseres Systems anhand des Bode-Diagramms, der Simulink-Schaltung und einer Handrechnung betrachet und anschließend verglichen.

Für die Statik-Information $y_{stat}$ gilt:
\begin{align*} 
    y_{stat} = |\mathcal{G}(j\omega_0)| \cdot A \cdot sin(\omega_0 t+\phi_0 + arg \enspace \mathcal{G}(j\omega))
\end{align*}

Dabei entspricht $|\mathcal{G}(j\omega_0)|$ der Amplitudenverstärkung und $arg \enspace \mathcal{G}(j\omega)$ der Phasenverschiebung.
...
\end{align*} 
Berechnung der Amplitudenverstärkung für $ \omega = 6$: \\
\begin{align*} 
...
$arg(\frac{a + bj}{c + dj}) = WinkelZ\ddot ahler - WinkelNenner = arctan\frac{d}{c} - arctan\frac{b}{a}$ \\
...
\begin{figure}[H]
    ...
    \caption{Bode-Plot mit Zusatzinformationen an der Stelle $ \omega = 6$}
 \end{figure}
...
\begin{figure}[H]
    ...
    \caption[width=0.6\textwidth]{Messung der Amplitude der eingeschwungnen Ausgansfunktion}
 \end{figure}
 Aus dem Simulink-Ergebnis lässt sich ablesen, dass die Amplitude bei eingeschwungener Ausgangsfunktion ungefähr 0.17 beträgt.
...
Somit ergibt sich bei unserem System eine Verstärkung von 1, weshalb die statische Kennlinie eine Gerade mit der Steigung 1 ist. 
...
Noch ein anderer Weg, die statische Versärkungsmatrix $K$ auszurechnen (im mehrvariablen Fall ist $K$ eine Matrix, die die jeweiligen statischen Verstärkung der einzelnen Eingangssignale angibt), verwendet die Zustandsmatrizen. \\
